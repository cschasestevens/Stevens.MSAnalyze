...
ms_plot_lipidnet <- function(
  ...
  ## Nodes
  net_nodes <- read.table(
    "ref/0.lipidnet.base.nodes.txt",
    header = TRUE,
    sep = "\t"
  )
  ## Edges
  net_edges <- read.table(
    "ref/0.lipidnet.base.edges.txt",
    header = TRUE,
    sep = "\t"
  )
  # Create network
  ...
  net_anno <- setNames(aggregate(
    unique(d[["chemrich"]][, c(4, 7, 11)])[["n"]], # nolint
    list(unique(d[["chemrich"]][, c(4, 7, 11)])[["label"]]),
    function(x) sum(x)
  ...
  net_nodes[["cluster.size"]] <- ifelse(
    is.na(net_nodes[["cluster.size"]]),
    0,
    net_nodes[["cluster.size"]]
  )
  net_nodes[["set.alpha"]] <- (
    (net_nodes[["cluster.size"]] - min(net_nodes[["cluster.size"]])) /
      (max(net_nodes[["cluster.size"]])) - min(net_nodes[["cluster.size"]])
  )
  net_nodes[["set.clus.size"]] <- (
    (net_nodes[["cluster.size"]] - min(net_nodes[["cluster.size"]])) /
      (max(net_nodes[["cluster.size"]])) - min(net_nodes[["cluster.size"]])
  )
  ## Adjust edge attributes
  net_edges[["set.alpha.edge"]] <- ifelse(
    net_edges[["from"]] %in%
      net_nodes[net_nodes[["set.clus.size"]] > 0, "Node"],
    1,
    0.25
  )
